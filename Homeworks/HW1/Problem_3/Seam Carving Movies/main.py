...
def linspace_blending(left_pixels, right_pixels):
    alpha = np.linspace(1, 0, num=len(left_pixels), endpoint=True)
    alpha = alpha.reshape(-1, 1)
    blended_pixels = alpha * left_pixels + (1 - alpha) * right_pixels
...
def build_composite(img_left, img_right, min_seam, count):
    ...
    for i in range(M):
        c_i = min_seam[i] + int(BLENDING_OFFSET//2)
        composite_img[i, :c_i - BLENDING_OFFSET] = img_left[i, :c_i - BLENDING_OFFSET]
        blended_pixels = linspace_blending(img_left[i, c_i - BLENDING_OFFSET:c_i + BLENDING_OFFSET],
                                           img_right[i, c_i - BLENDING_OFFSET:c_i + BLENDING_OFFSET])
        composite_img[i, c_i - BLENDING_OFFSET:c_i + BLENDING_OFFSET] = blended_pixels
        composite_img[i, c_i + BLENDING_OFFSET:] = img_right[i, c_i + BLENDING_OFFSET:]
        # composite_img[i, c_i] = (255, 0, 0)
...
def find_min_seam(min_costs):
    ...
    for i in range(M - 2, -1, -1):
        tmp_min_cost = np.argmin(
            [min_costs[i, column_index], min_costs[i, column_index - 1], min_costs[i, column_index + 1]])
        if (tmp_min_cost == 1):
            column_index -= 1
        elif (tmp_min_cost == 2):
            column_index += 1
...
def main():
    ...
    writer = cv2.VideoWriter('./outputs/twinning.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps_left,
